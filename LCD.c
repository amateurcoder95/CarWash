...
void DisplayTimeToLCD(unsigned char* pTimeArray) // Displays time in HH:MM:SS AM/PM format
{
    ...
    // Display Hour
    if (pTimeArray[2] % 10 == 0) {
        WriteCommandToLCD(0x80);
        WriteDataToLCD((pTimeArray[2] / 10) + 0x30);
    }
    if (pTimeArray[1] / 10 == 0) {
        WriteCommandToLCD(0x80 + 1);
        WriteDataToLCD((pTimeArray[2] % 10) + 0x30);
    }
    //Display ':'
    WriteCommandToLCD(0x80 + 2);
    WriteDataToLCD(':');

    //Display Minutes
    if (pTimeArray[1] % 10 == 0) {
        WriteCommandToLCD(0x80 + 3);
        WriteDataToLCD((pTimeArray[1] / 10) + 0x30);
    }
    if (pTimeArray[0] / 10 == 0) {
        WriteCommandToLCD(0x80 + 4);
        WriteDataToLCD((pTimeArray[1] % 10) + 0x30);
    }
    //Display ':'
    WriteCommandToLCD(0x80 + 5);
    WriteDataToLCD(':');

    //Display Seconds
    if (pTimeArray[0] % 10 == 0) {
        WriteCommandToLCD(0x80 + 6);
        WriteDataToLCD((pTimeArray[0] / 10) + 0x30);
    }
    WriteCommandToLCD(0x80 + 7);
    WriteDataToLCD((pTimeArray[0] % 10) + 0x30);
    ...
    // Display mode
    switch (pTimeArray[3]) {
        case AM_Time: WriteStringToLCD("AM");
            break;
        case PM_Time: WriteStringToLCD("PM");
            break;

        default: WriteDataToLCD('H');
...
void WriteTempToLCD(int Temp) {
    //WriteCommandToLCD(0xc0);
    WriteDataToLCD(Temp / 10 + 0x30);
    WriteDataToLCD(Temp % 10 + 0x30);
}

void DisplayFirstTimeToLCD(unsigned char* pTimeArray) // Displays time in HH:MM:SS AM/PM format
{
    ClearLCDScreen(); // Move cursor to zero location and clear screen
